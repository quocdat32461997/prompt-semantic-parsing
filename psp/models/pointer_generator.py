...
from psp.constants import ParseOutputs, RunMode


class PointerGenerator(torch.nn.Module):
    def __init__(self, vocab_size: int, ontology_vocab_ids: List[int],
                 input_dim: int, hidden_dim_list: List[int],
                 num_heads: int = 12, dropout: float = 0.3) -> None:
        """
        Args:
        ...
        self.ontology_vocab_ids: Tensor = torch.tensor(ontology_vocab_ids)  # [ontology_vocab]
    ...
    def forward(self, source_input_ids: Tensor, encoder_hidden_states: Tensor, encoder_attn_mask: Tensor,
                decoder_hidden_states: Tensor, run_mode: RunMode) -> Tensor:
        """
        Args:
        ...
        ontology_probs: Tensor = softmax_func(ontology_logits)  # [batch_size, max_seq_len or 1, ontology_vocab_size]
        copy_probs: Tensor = softmax_func(copy_logits)  # [batch_size, max_seq_len or 1, 1]

        # Get final probs
        copy_source_probs *= copy_probs  # [batch_size, max_seq_len or 1, source_seq_len]
        ontology_probs *= (1 - copy_probs)  # [batch_size, max_seq_len or 1, ontology_vocab_size]

        # Parse into full vocabs
        # [batch_size, max_seq_lne or 1, vocab_size]
        vocab_probs: Tensor = torch.zero(copy_probs.shape[:2] + [self.vocab_size], dtype=copy_probs.dtype)

        source_input_ids = source_input_ids.unsqueeze(1)   # [batch_size, 1, source_seq_len]
        # [batch_size, max_seq_len or 1, source_seq_len]
        source_input_ids = torch.tile(source_input_ids, [1, copy_probs.shape[1], 1])
        vocab_probs = vocab_probs.scatter_add_(dim=-1, index=source_input_ids, src=copy_source_probs)

        ontology_vocab_ids: Tensor = self.ontology_vocab_ids.reshape([1, 1, -1])    # [1, 1, ontology_vocab]
        # [batch_size, max_seq_len or 1, ontology_vocab]
        ontology_vocab_ids = torch.tile(ontology_vocab_ids, vocab_probs.shape[:2] + [1])
        vocab_probs = vocab_probs.scatter_add_(dim=-1, index=self.ontology_vocab_ids, src=ontology_probs)
